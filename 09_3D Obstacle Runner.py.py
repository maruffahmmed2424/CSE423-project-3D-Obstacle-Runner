...
def draw_coin(x, y, z, radius=5.0, color=(1.0, 0.8, 0.0)):
    quad = gluNewQuadric()
    glPushMatrix()
    glTranslatef(x, y + 0.5, z)
    glColor3f(*color)
...
def draw_lanes():
    ...
    for i in range(3):
        x = lane_positions[i]
        # LEFT black lane (mirror of right)
        glBegin(GL_QUADS)
        glColor3f(0.0, 0.0, 0.0)
        glVertex3f(-lane_width*2.2, 0, -GRID_LENGTH)
        glVertex3f(-lane_width*1.5, 0, -GRID_LENGTH)
        glVertex3f(-lane_width*1.5, 0, GRID_LENGTH)
        glVertex3f(-lane_width*2.2, 0, GRID_LENGTH)
        glEnd()
...
#oishy
def setupCamera():
    global fppView, lane, jump_height, camheight, camdist, campitch
...
def generate_obstacles():
    ...
    if random.random() < 0.08 and len(coins) < 10:
        lane_choice = random.choice([-1,0,1])
        coins.append({'x': lane_choice * lane_width,'y': 50,
                      'z': -300,'radius': 5,'color': random_ball_color()})
...
def calculate_distance(x, y, z):
    ...
    camera_pos = [0, camheight, camdist]
    obj_pos = [x, y, z]
        ...
        if obj_type == "obstacle":
            ...
            for coin in coins:
                if coin['x'] == x and coin['y'] == y and coin['z'] == z:
                    draw_coin(coin['x'],coin['y'],coin['z'],coin['radius'],coin['color']
)
                    break
        elif obj_type == "env":
            for obj in environment_objects:
...
def showScreen():
    ...
    draw_small_ui_text(10,745,"R: Start  S: Stop  Space: Jump ")
    ...
    if game_over:
        draw_black_ui_text(400, 400, "GAME OVER")
        draw_black_ui_text(420, 370, f"Final Score: {int(score)}")
        draw_black_ui_text(420, 350, "Press R to restart")
...
def main():
    ...
    glutCreateWindow(b"3D Obstacle Runner -")
